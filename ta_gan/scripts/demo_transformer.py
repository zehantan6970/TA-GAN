...
def main(args):
    ...
    for batch in val_loader:
        ...
        then = time.time()
        print('每条轨迹平均预测时间：{:.3f}s'.format((then-now)/obs_traj.size(1)))
        print('预测的20个轨迹点坐标')
        output = torch.transpose(output, 0, 1).cpu().detach().numpy()
        print(output.reshape(obs_traj.size(1), -1))
        ...
        c_size = output.shape[0]
        color = numpy.linspace(1, c_size, c_size).reshape(c_size,1)
        color = numpy.repeat(color, args.obs_len, 1)
        plt.clf() #清空画布
        plt.xlim(-2.5, 2.5)
        plt.ylim(-2.5, 2.5)
        #fig = plt.figure()
        plt.scatter(output_x, output_y, s=17, c=color, marker='.')
        plt.scatter(obs_traj_x, obs_traj_y, s=17, c=color, marker='+')
        plt.scatter(pred_traj_gt_x, pred_traj_gt_y, s=17, c=color, marker='d')
        plt.legend(['pred', 'obs', 'ground_truth'])
        #plt.show()
