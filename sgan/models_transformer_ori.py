...
class Trajectory_Generator(nn.Module):
    ...
    def add_noise(self, input, noise=None):
        """Append latent noise without assuming a particular CUDA device.

        ``noise`` is optional and exists for reproducible evaluation.  It does
        not add parameters or change the historical checkpoint schema.
        """
        if self.noise_dim == 0:
            return input
        ...
        if noise is None:
            noise = torch.randn(
                *shape, device=input.device, dtype=input.dtype
            )
        else:
            ...
            noise = noise.to(device=input.device, dtype=input.dtype)
        return torch.cat([input, noise], dim=-1)
    ...
    def forward(self, obs_traj, obs_traj_rel, seq_start_end, noise=None):
        ...
        noise_output = self.add_noise(merge_output, noise=noise)
